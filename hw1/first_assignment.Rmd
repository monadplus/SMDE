...
On the _X100m_ we can refuse the null hypothesis $H_0$ because the p-value 0.00407 is smaller than $\alpha$. So we can state with 95% of confidence that there is difference in average in the times for the 100 meters race dependening on the competition.

```{r}
t.test(X100m ~ Competition, data = decathlon)
```

On the _X400m_ we can't refuse the null hypothesis $H_0$ because the p-value 0.9543 is greater than $\alpha$. So there is no significative effidence that the average time on the 400 meters race is different depending on the competition.
