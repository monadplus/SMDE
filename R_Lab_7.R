...
library(Rcmdr) # RcmdrMisc.readXL
...
food <- readXL("/home/arnau/MIRI/SMDE/pca/food.xls"
...
# The p-value of Bartlett test is 0.001646 < 0.05. 
# Then the null hypothesis is rejected. The variables are correlated.

## Kaiser-Meyer-Olkin (KMO) Test

# We can use kmo function written by Prof. Shigenobu Aok (http://minato.sip21c.org/swtips/factor-in-R.pdf)
kmo <- function(x)
{
...
# 1st Component = indictor of Orange prices
# 2nd Component = second component is the indicator of the prices of the rest of the aliments.
...
library(FactoMineR)
res_PCA<-PCA(food,scale=TRUE, graph=TRUE) # by default scale=TRUE
...
domes <- readXL( "/home/arnau/MIRI/SMDE/pca/domes.xls"
               , rownames=FALSE
               , header=TRUE
               , na=""
               , sheet="Sayfa1"
...
# For the detailed information please check the Word document (/home/arnau/MIRI/SMDE/pca/info_domes.docx)
